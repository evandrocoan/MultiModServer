...
public plugin_init()
{
	register_plugin("CS Misc. Stats UT", AMXX_VERSION_STR, "Ori, Amx Mod X Dev Team")
...
public plugin_cfg()
{
	...
	server_cmd(g_addStast, "Multi-Kill Text", "MultiKill")
	server_cmd(g_addStast, "Multi-Kill Sound", "MultiKillSound")
	server_cmd(g_addStast, "Bamb Planting Text", "BombPlanting")
	server_cmd(g_addStast, "Bomb Defusing Text", "BombDefusing")
	server_cmd(g_addStast, "Bomb Planted Text", "BombPlanted")
	server_cmd(g_addStast, "Bomb Defused Text", "BombDefused")
	server_cmd(g_addStast, "Bomb Failed Defuse Text", "BombFailed")
	server_cmd(g_addStast, "Bomb Pick Up Text", "BombPickUp")
	server_cmd(g_addStast, "Bomb Drop Text", "BombDrop")
	server_cmd(g_addStast, "Bomb Count Voice", "BombCountVoice")
	server_cmd(g_addStast, "Bomb Count Defuse Text", "BombCountDef")
	server_cmd(g_addStast, "Bomb Side Reached Text", "BombReached")
	server_cmd(g_addStast, "Italy Bonus Kill Text", "ItalyBonusKill")
	server_cmd(g_addStast, "Last Man Text, Sound", "LastMan")
	server_cmd(g_addStast, "Knife Kill Text", "KnifeKill")
	server_cmd(g_addStast, "Knife Kill Sound", "KnifeKillSound")
	server_cmd(g_addStast, "Granade Kill Text", "GrenadeKill")
	server_cmd(g_addStast, "Granade Suicide Text", "GrenadeSuicide")
	server_cmd(g_addStast, "Head Shot Text", "HeadShotKill")
	server_cmd(g_addStast, "Head Shot Kill Sound", "HeadShotKillSound")
	server_cmd(g_addStast, "Round Counter Text", "RoundCounter")
	server_cmd(g_addStast, "Killing Streak Text", "KillingStreak")
	server_cmd(g_addStast, "Killing Streak Sound", "KillingStreakSound")
	server_cmd(g_addStast, "Enemy Remaining Text", "EnemyRemaining")
	server_cmd(g_addStast, "Double Kill Text", "DoubleKill")
	server_cmd(g_addStast, "Double Kill Sound", "DoubleKillSound")
	server_cmd(g_addStast, "First Blood Sound", "FirstBloodSound")
	server_cmd(g_addStast, "Player name Text", "PlayerName")
}
...
public client_death(killer, victim, wpnindex, hitplace, TK)
{
	...
	if ((KillingStreak || KillingStreakSound) && !TK)
	{
		...
		if (!selfkill)
		{
			...
			new a = g_streakKills[killer][0] - 5
...
public eNewRound()
{
	if (read_data(1) == floatround(get_cvar_float("mp_roundtime") * 60.0,floatround_floor))
	{
		...
		if (RoundCounter)
		{
			set_hudmessage(250, 0, 250, -1.0, 0.30, 0, 6.0, 6.0, 0.5, 0.15, -1)
