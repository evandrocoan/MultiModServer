...
#define FLAG_PSAY ADMIN_BAN 								/* Players with this flag can read others players' private messages */
...
new g_HudColors[][] = {"white", "red", "green", "blue", "yellow", "magenta", "cyan", "orange", "ocean", "maroon"}
new g_HudValues[][] = {{255, 255, 255}, {255, 0, 0}, {0, 255, 0}, {0, 0, 255}, {255, 255, 0}, {255, 0, 255}, {0, 255, 255}, {227, 96, 8}, {45, 89, 116}, {103, 44, 38}}
...
public cmd_say(id, level, cid)
{
	...
	formatex(message, charsmax(message), "%s", get_message(id, 0, is_anonymous() ? g_AnonymousMessages[CMD_SAY] : g_Messages[CMD_SAY], arg))
	
	new players[32], num
	get_players(players, num)
	
	for(new i = 0; i < num; i++)
		ColorChat(players[i], TEAM_COLOR, message)
...
public cmd_asay(id, level, cid)
{
	...
	new message[192], name[32]
	get_user_name(id, name, charsmax(name))
	formatex(message, charsmax(message), "%s", get_message(id, 0, is_anonymous() ? g_AnonymousMessages[CMD_ASAY] : g_Messages[CMD_ASAY], arg))
		
	new players[32], num
	get_players(players, num)
	
	for(new i = 0; i < num; i++)
		if(get_user_flags(players[i]) & FLAG_READ || id == players[i]) ColorChat(players[i], TEAM_COLOR, message)
...
public cmd_chat(id, level, cid)
{
	...
	new message[192], name[32]
	get_user_name(id, name, charsmax(name))
	formatex(message, charsmax(message), "%s", get_message(id, 0, is_anonymous() ? g_AnonymousMessages[CMD_CHAT] : g_Messages[CMD_CHAT], arg))
		
	new players[32], num
	get_players(players, num)
	
	for(new i = 0; i < num; i++)
		if(get_user_flags(players[i]) & FLAG_VIPCHAT) ColorChat(players[i], TEAM_COLOR, message)
...
public cmd_psay(id, level, cid)
{
	...
	new message[192], name[32], name2[32]
	get_user_name(id, name, charsmax(name))
	get_user_name(player, name2, charsmax(name2))
	formatex(message, charsmax(message), "%s", get_message(id, player, is_anonymous() ? g_AnonymousMessages[CMD_PSAY] : g_Messages[CMD_PSAY], arg))
		
	new players[32], num
	get_players(players, num)
	
	for(new i = 0; i < num; i++)
		if(get_user_flags(players[i]) & FLAG_PSAY || player == players[i]) ColorChat(players[i], TEAM_COLOR, message)
...
public cmd_hsay(id, level, cid)
{
	...
	new message[192], name[32], command[10], type, clr
	get_user_name(id, name, charsmax(name))
	read_argv(0, command, charsmax(command))
	formatex(message, charsmax(message), "%s", get_message(id, 0, is_anonymous() ? g_AnonymousMessages[CMD_HSAY] : g_Messages[CMD_HSAY], arg))
	...
	new bool:dhud = (command[8] == '2') ? true : false
...
stock get_message(id, player, msg[], arg[])
{
	...
	{
		get_user_name(id, info, charsmax(info))
...
stock is_anonymous()
	return (get_pcvar_num(g_Anonymous)) ? true : false
