...
//=========================================================
float	CMTalkMonster::g_talkWaitTime = 0;		// time delay until it's ok to speak: used so that two NPCs don't talk at once

// NOTE: m_voicePitch & m_szGrp should be fixed up by precache each save/restore

// array of friend names
char *CMTalkMonster::m_szFriends[TLK_CFRIENDS] = 
{
	"monster_barney",
	"monster_scientist",
...
DEFINE_CUSTOM_SCHEDULES( CMTalkMonster )
{
	slIdleResponse,
	slIdleSpeak,
	slIdleHello,
...
IMPLEMENT_CUSTOM_SCHEDULES( CMTalkMonster, CMBaseMonster );


void CMTalkMonster :: SetActivity ( Activity newActivity )
{
...
void CMTalkMonster :: RunTask( Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_TLK_CLIENT_STARE:
	case TASK_TLK_LOOK_AT_CLIENT:
		...
		if ( m_MonsterState == MONSTERSTATE_IDLE		&& 
			 ...
			 !IsTalking()								)
		{
			// Get edict of nearest player
         pPlayer = UTIL_FindNearestPlayer(this->edict(), m_flFieldOfView);
	...
	case TASK_FACE_PLAYER:
		{
			// Get edict of nearest player
			edict_t *pPlayer = UTIL_FindNearestPlayer(this->edict(), m_flFieldOfView);
	...
	case TASK_TLK_EYECONTACT:
		if (!IsMoving() && IsTalking() && m_hTalkTarget != NULL)
		{
			...
			IdleHeadTurn( m_hTalkTarget->v.origin );
		}
		else
		{
			TaskComplete();
...
void CMTalkMonster::AlertFriends( void )
{
/*jlb
	edict_t *pFriend = NULL;
	int i;
	...
	for ( i = 0; i < TLK_CFRIENDS; i++ )
	{
		...
		{
			CMBaseMonster *pMonster = pFriend->MyMonsterPointer();
			if ( pMonster->IsAlive() )
			{
...
void CMTalkMonster::ShutUpFriends( void )
{
/*jlb
...
void CMTalkMonster::LimitFollowers( edict_t *pPlayer, int maxFollowers )
{
/*jlb
	CBaseEntity *pFriend = NULL;
	int i, count;
...
float CMTalkMonster::TargetDistance( void )
{
	// If we lose the player, or he dies, return a really large distance
	if ( m_hTargetEnt == NULL || !UTIL_IsAlive(m_hEnemy) )
		return 1e6;

	return (m_hTargetEnt->v.origin - pev->origin).Length();
}
...
//=========================================================
void CMTalkMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	CMBaseMonster::HandleAnimEvent( pEvent );
}

// monsters derived from CMTalkMonster should call this in precache()

void CMTalkMonster :: TalkInit( void )
{
...
// nearest player
//=========================================================
edict_t *CMTalkMonster :: FindNearestFriend(BOOL fPlayer)
{
	edict_t *pFriend = NULL;
...
int CMTalkMonster :: GetVoicePitch( void )
{
...
//=========================================================
void CMTalkMonster :: IdleRespond( void )
{
...
int CMTalkMonster::CanPlaySentence( BOOL fDisregardState ) 
{ 
	if ( fDisregardState )
		return CMBaseMonster::CanPlaySentence( fDisregardState );
	return FOkToSpeak(); 
}
...
void CMTalkMonster :: IdleHeadTurn( Vector &vecFriend )
{
...
//=========================================================
int CMTalkMonster :: FIdleSpeak ( void )
{ 
	...
	if ( pTarget != NULL )
	{
		...
		{
			if ( UTIL_IsAlive(pTarget) )
			{
				m_hTalkTarget = m_hTargetEnt;
				if (!FBitSet(m_bitsSaid, bit_saidDamageHeavy) && 
					(m_hTargetEnt->v.health <= m_hTargetEnt->v.max_health / 8))
				{
					...
					(m_hTargetEnt->v.health <= m_hTargetEnt->v.max_health / 4))
				{
					...
					(m_hTargetEnt->v.health <= m_hTargetEnt->v.max_health / 2))
				{
	...
	if (pFriend && !(UTIL_IsMoving(pFriend)) && (RANDOM_LONG(0,99) < 75))
	{
		...
		pTalkMonster->SetAnswerQuestion( this->edict() ); // UNDONE: This is EVIL!!!
	...
	if ( RANDOM_LONG(0,1) )
	{
		...
		edict_t *pFriend = FindNearestFriend(TRUE);
...
void CMTalkMonster::PlayScriptedSentence( const char *pszSentence, float duration, float volume, float attenuation, BOOL bConcurrent, edict_t *pListener )
{
	if ( !bConcurrent )
		ShutUpFriends();
...
void CMTalkMonster::PlaySentence( const char *pszSentence, float duration, float volume, float attenuation )
{
	if ( !pszSentence )
		return;
	...
	CMTalkMonster::g_talkWaitTime = gpGlobals->time + duration + 2.0;
	if ( pszSentence[0] == '!' )
...
// talking.
//=========================================================
void CMTalkMonster :: Talk( float flDuration )
{
	if ( flDuration <= 0 )
	{
		// no duration :( 
...
void CMTalkMonster :: SetAnswerQuestion( edict_t *pSpeaker )
{
...
Schedule_t* CMTalkMonster :: GetScheduleOfType ( int Type )
{
	switch( Type )
	{
	...
	case SCHED_IDLE_STAND:
		{	
			...
			if (!FBitSet(m_bitsSaid, bit_saidWoundLight) && (pev->health <= (pev->max_health * 0.75)))
			{
				...
				//CMTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT(2.8, 3.2);
			...
			// NOTE - caller must first CMTalkMonster::GetScheduleOfType, 
...
//=========================================================
BOOL CMTalkMonster :: IsTalking( void )
{
...
//=========================================================
void CMTalkMonster :: PrescheduleThink ( void )
{
...
void CMTalkMonster :: TrySmellTalk( void )
{
	if ( !FOkToSpeak() )
		return;
...
int CMTalkMonster::IRelationship( CMBaseEntity *pTarget )
{
	if ( pTarget->IsPlayer() )
		if ( m_afMemory & bits_MEMORY_PROVOKED )
			return R_HT;
...
void CMTalkMonster::StopFollowing( BOOL clearSchedule )
{
	if ( IsFollowing() )
	{
		if ( !(m_afMemory & bits_MEMORY_PROVOKED) )
...
void CMTalkMonster :: FollowerUse( edict_t *pActivator, edict_t *pCaller, USE_TYPE useType, float value )
{
	// Don't allow use during a scripted_sentence
	if ( m_useTime > gpGlobals->time )
		return;

	if ( pCaller != NULL && UTIL_IsPlayer(pCaller) )
	{
...
void CMTalkMonster::KeyValue( KeyValueData *pkvd )
{
...
void CMTalkMonster::Precache( void )
{
	if ( m_iszUse )
