...
bool EngineInfo::check_for_engine_module( const char* _pName )
{
	const char* pC;
	size_t size;
...
int EngineInfo::nthdr_module_name( void )
{
...
void EngineInfo::set_code_range( unsigned char* _pBase, PIMAGE_NT_HEADERS _pNThdr )
{
...
int EngineInfo::phdr_dladdr( void* _pMem )
{
	Dl_info info;
...
int EngineInfo::phdr_r_debug( void )
{
    ElfW(Dyn)* pDyn; 
...
void EngineInfo::set_code_range( void* _pBase, ElfW(Phdr)* _pPhdr )
{
...
int EngineInfo::initialise( enginefuncs_t* _pFuncs )
{
	int ret = 0;
