...
bool BotFindBreakable (bot_t *pBot)
{
   ...
   for (int i = 0; i < 5; ++i)
   {
             ...
             && (tr.pHit->v.takedamage > 0) && ((tr.pHit->v.impulse == 0) || (i == 0))))) // KWo - 12.03.2010
      {
         ...
         if (g_b_DebugEntities)
         {
            ALERT(at_logged, "[DEBUG] Bot %s found a breakable from position %i ; wpt_index = %i; previous index = %i.\n",
               pBot->name, i, pBot->curr_wpt_index, pBot->prev_wpt_index[0]);
            if (i>=0)
...
//                  UTIL_ClampVector (&pBot->target_angular_speed);
         ...
         else
         {
            ...
            if (((pBot->dest_origin - pBot->pEdict->v.origin).Length() <= 15) && (pBot->bMoveToGoal)
               || (BotGetSafeTask(pBot)->iTask == TASK_PAUSE)) // KWo - 12.03.2010
            {
...
//      ALERT(at_logged, "[DEBUG] BotThink - Bot %s has task deep = %i, path deep = %i.\n", pBot->name, pBot->i_TaskDeep, pBot->i_PathDeep);
   ...
   if (pBot->bMoveToGoal)
   {
      ...
      if (((paths[pBot->curr_wpt_index]->flags & W_FL_CROUCH))
          ...
          && (((pEdict->v.flags & FL_ONGROUND) && !pBot->bInWater) // KWo - test
      ...
      {
            // Slowly approach the ladder
         pBot->f_move_speed = (pEdict->v.origin - pBot->wpt_origin).Length ();

         if (pBot->f_move_speed < 60.0)
            pBot->f_move_speed = 60.0;
         else if (pBot->f_move_speed > pEdict->v.maxspeed)
            pBot->f_move_speed = pEdict->v.maxspeed;
 //        if (bPrevLadder)
 //           pBot->f_move_speed = 60.0;    // KWo - 24.01.2012
      }

      if ((pBot->cCollisionState == COLLISION_NOTDECIDED) && (fDistance < pBot->f_move_speed * pBot->fTimeFrameInterval))  // KWo - 08.01.2008
         pBot->f_move_speed = fDistance;

      if ((pBot->cCollisionState == COLLISION_NOTDECIDED) && (pBot->f_move_speed != 0.0)) // KWo - 20.04.2013
      {
         if (pBot->f_wpt_tim_str_chg + 0.5 > gpGlobals->time) // KWo - 08.03.2010
         {
         ...
         {
            if ((pBot->curr_wpt_index != BotGetSafeTask(pBot)->iData) && (pBot->f_move_speed < 60.0))
               pBot->f_move_speed = 60.0;
...
*/
        ...
        && (pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints))
   {
      ...
      if (pBot->pEdict->v.maxspeed <= 0.0)
         ...
         fReachWpDelay = 5.0 * fDistance / pBot->pEdict->v.maxspeed;
